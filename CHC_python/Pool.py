...
class pool:
    def __init__(self, pool_size_, string_length_, number_genes_in_pool_=None):
        self.genes_array = []
        self.pool_size = pool_size_
        self.string_length = string_length_
        if number_genes_in_pool_:
            self.number_genes_in_pool = number_genes_in_pool_
        else:
            self.number_genes_in_pool = pool_size_
    ...
    def init_pool(
        ...
        bias=0,
        init_from_data_file=None,
    ):
        if not init_from_data_file:
            if bias == 0:
                print("Start initialize random pool")
                num_init = self._init_random_pool(
                    start_index, stop_index, translation_function
                )
            else:
                print("Start initialize bias pool")
                num_init = self._init_bias_pool(
                    bias, start_index, stop_index, translation_function
                )

        else:
            print("Start initialize pool from a file")
            try:
                file = open(init_from_data_file, "r")
            except OSError:
                logging.ERROR(
                    "Can not open pool initialization file: {}".format(
                        init_from_data_file
                    )
                )
                exit()

            num_init = self._init_pool_from_file()
            file.close()

            num_init = self._validate_pool_from_file(
                num_init, start_index, stop_index, translation_function
            )

        return num_init

    def _validate_pool_from_file(
        self, num_init, start_index, stop_index, translation_function
    ):
        if num_init < 0:
            logging.ERROR("Init pool: bad initialization indices")
            exit()

        if num_init != stop_index - start_index:
            # If file did not contain enough initialization values, finish off with random initialization
            logging.WARNING(
                "{} genes read from file.\n {} genes will be randomly generated\n".format(
                    num_init, stop_index - num_init
                )
            )
            rand_num_init = self._init_random_pool(
                start_index, stop_index, translation_function
            )
    ...
    def _init_random_pool(self, start_index, stop_index, translation_function):
        ...
        for index in range(start_index, stop_index):
            tmp_gene = Gene.gene(
                init_type, self.string_length, translation_function
            )  # if I want to init non binary population, I should change the 2
    ...
    def _init_bias_pool(self, bias, start_index, stop_index, translation_function):
        # Init Type:
        # binary = 2
        # scale = N
        ...
        for index in range(start_index, stop_index):
            tmp_gene = Gene.gene(
                init_type, self.string_length, translation_function, 1, bias
            )
            self.genes_array.append(tmp_gene)
        return stop_index - start_index
    ...
    def _init_pool_from_file(self):

        print(
            "Initialized from file isn't implement yet. Genes will generated randomly"
        )
        return 0
    ...
    def insert_gene(self, new_gene):
        if len(new_gene.string) != self.string_length:
            logging.error(
                "The new gene doesn't have the same string length as the genes in the pool"
            )
            exit(1)
        if len(self.genes_array) >= self.pool_size:
            logging.error(
                "The pool is full (Poll size: {}), can't insert new gene".format(
                    self.pool_size
                )
            )
            exit(1)
        self.genes_array.append(new_gene)
        self.number_genes_in_pool = +1

    def replace_new_gene_by_worth_val(self, new_gene, opt_operator):
        if not (
            selecation_functions.find_optimum(
                new_gene.gene_worth,
                self.genes_array[self.pool_size - 1].gene_worth,
                opt_operator,
            )
        ):
            return
