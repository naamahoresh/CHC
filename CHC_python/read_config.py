...
def read_config(file_name):
    ...
    if not (function[0] == 'f'):
        logging.error("Need to implement function with noise, composition of functions and dual function evaluation")
        exit(1)
    ...
    for index in range(Nb_params):
        ...
        if params_list[2]=="u":
            ...
            sign=1
        dic={"param_name":params_list[0],"length":int(params_list[1]),"sign":sign,"scale":float(params_list[3]), "wrap_low":float(params_list[4]),"wrap_high":float(params_list[5]), "position":postn}
        postn += dic['length']
