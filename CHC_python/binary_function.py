...
def ham_dist(array1, array2, array_length, Work_params):
    distance = 0
    for index in range(array_length):
        delta1 = char_to_double(
            array1[Work_params[index].posn], Work_params[index].lngth, 1, 0
        )  # NO_CHANGE, UNSIGNED
        delta2 = char_to_double(
            array2[Work_params[index].posn], Work_params[index].lngth, 1, 0
        )  # NO_CHANGE, UNSIGNED
...
def degray(array_in, array_length, Work_params):
    ...
    for index in range(array_length):
        ...
        for index2 in range(lngth):
            ...
            last = array_in[
                index2
            ]  ##check in the code. it write: last= array_out[index2]
...
def sym_trnsfrm(array, num_params, Work_params):
    ...
    for index in range(num_params):
        values[index] = char_to_double(
            array,
            Work_params[index].position,
            Work_params[index].length,
            Work_params[index].scale,
            Work_params[index].sign,
        )

        buff_index += Work_params[index].length
        max = math.pow(2, Work_params[index].length)
        values[index] = (
            (values[index] * Work_params[index].wrap_high) / (max / 2.0)
        ) - Work_params[index].wrap_high
