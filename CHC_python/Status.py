...
def show_progress_exp(pool, current_generation, result_file):
    ...
    output_string = "{:3d}            {:.4f}      {:.4f}      {:.4f}      {:.4f}     \n".format(
        current_generation,
        pool.genes_array[0].gene_worth,
        pool.genes_array[int(pool.pool_size - 1)].gene_worth,
        pool.genes_array[int(pool.pool_size / 2)].gene_worth,
        avr_worth,
    )
...
def status_dump(pool, file_name, user_args, trail):
    ...
    try:
        ...
        logging.error(
            "Cannot open file to save the pool config: {}".format(file_config)
        )
        exit(1)
...
def save_params(work_params, num_params, result_file):
    ...
    for param in range(num_params):
        file.write(
            (
                "Param Name: {} -- {}\n".format(
                    work_params[param]["param_name"], work_params[param]
                )
            )
        )
    file.write("\n\n")
...
def save_pool(pool, trail):
    ...
    for gene_index in range(pool.pool_size):
        ...
        string = string + "Gene num: {}    Worth: {}    String: {}\n".format(
            str(gene_index), str((pool.genes_array[gene_index].gene_worth)), string_gene
        )
    return string
...
def final_report(dict_best_result, file_name):
    ...
    str_final = "Max solution: {} ({} exp found this result)\nMax String: {}\nAverage: {}\nMedian: {}".format(
        max(worth_list),
        Counter(worth_list)[max(worth_list)],
        string_list[worth_list.index(max(worth_list))],
        statistics.mean(worth_list),
